...
function writeBlock(f, buffer, blockNumber, pad = false, padding = 0x00) {
	fs.writeSync(f.fd, buffer, 0, buffer.length, f.offset + blockNumber * f.blockSize);
	...
	if (buffer.length < f.blockSize) {
		...
		fs.writeSync(f.fd, Buffer.alloc(remaining, padding), 0, remaining, f.offset + blockNumber * f.blockSize + buffer.length);
	}
}

function readBlock(f, blockNumber) {
	let buf = Buffer.alloc(f.blockSize);
	fs.readSync(f.fd, buf, 0, f.blockSize, f.offset + blockNumber * f.blockSize);
	return buf;
}
...
function writeInode(f, inode, inodeNumber) {
	...
	let offset = f.offset + (f.gds[group].bg_inode_table * f.blockSize) + (index - 1) * f.s.inode_size;
...
function writeFileFromBuffer(f, path, buffer, options = {}) {
	...
	let written = 0;
	slices.forEach(s => {
		fs.writeSync(f.fd, buffer, written, f.offset + Math.min(s[1] * f.blockSize, buffer.length - written), s[0] * f.blockSize);
		written += s[1];
	});
}
...
async function writeFileFromHostFileSystem(f, destinationPath, sourcePath, options = {}) {
	...
	for (let i = 0; i < slices.length; i++) {
		...
		const writeStream = fs.createWriteStream(null, {fd: f.fd, start: f.offset + s[0] * f.blockSize, autoClose: false});
...
function initExt2(fd, partitionSize, blockSize = 1024, options = {}) {
	...
	const f = {
		...
		blockSize: blockSize,
		offset: options.offset // partition offset
	};
	...
	createDirectory(f, '/lost+found', {prereservedDirectoryBlocks: 12, accessRights: 0700, time: options.time});
