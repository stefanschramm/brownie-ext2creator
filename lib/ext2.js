...
function writeFileFromBuffer(f, path, buffer, options = {}) {
	...
	let slices = createFileInodeAndGetDataSlices(f, path, buffer.length, options);
...
async function writeFileFromHostFileSystem(f, destinationPath, sourcePath, options = {}) {
	...
	// TODO: check sb if there are enough free blocks in file system
	let slices = createFileInodeAndGetDataSlices(f, destinationPath, size, options);

	let written = 0;
	for (let i = 0; i < slices.length; i++) {
		let s = slices[i];
		let readStream = fs.createReadStream(sourcePath, {start: written, end: written + s[1] * f.blockSize - 1}); // TODO: -1 correct?
		let writeStream = fs.createWriteStream(null, {fd: f.fd, start: s[0] * f.blockSize, autoClose: false});
...
function createFileInodeAndGetDataSlices(f, path, size, options) {
	...
	let slices = allocNextFreeBlocks(f, Math.ceil(size / f.blockSize));
...
function createDirectory(f, path, options = {}) {
	...
	// TODO: update link count in parent?
}
